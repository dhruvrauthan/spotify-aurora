...
vec4 aurora(vec3 ro, vec3 rd)
{
    ...
    for(float i=0.;i<50.;i++)
    {
		...
		// 3 hues blended
		// float hueA = phaseHue;   // greenish
		// float hueB = 0.45;  // aqua
		// float hueC = 0.6;   // blue

		// // We'll get t in [0..1] from the sine
		// float t = sin(i * 0.043) * 0.5 + 0.5;

		// // If you want a simple “triangle” wave across 3 hues:
		// if (t < 0.5) {
		// 	// blend from hueA to hueB
		// 	float tt = t / 0.5; 
		// 	col2.rgb = mix(hue2rgb(hueA), hue2rgb(hueB), tt) * rzt;
		// } else {
		// 	// blend from hueB to hueC
		// 	float tt = (t - 0.5) / 0.5;
		// 	col2.rgb = mix(hue2rgb(hueB), hue2rgb(hueC), tt) * rzt;
		// }
